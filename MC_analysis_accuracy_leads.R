...
initialize <- function()
{
    ...
    package.args <- c("ggplot2", "crayon", "dplyr", "plyr", "tidyr") 
...
main <- function()
{
    ...
    #combined_groups <- rbind(group_1, group_2)
    
    # add the accuracy per ECG for the two groups
    g1 <- addAccuracyGroup(group_1, coded_answers, 1)
    g2 <- addAccuracyGroup(group_2, coded_answers, 2)
    combined_groups <- rbind(g1, g2)
    ...
    for(i in 1:length(getStimuliList()))
    {
        ...
        current_stimuli <- combined_groups %>% filter(MediaName == current_stimuli_metadata$file_name) 
        
        
        
        chain_1 <- current_stimuli %>% filter(Accuracy == 1) %>% select(c("participant", "AOI"))
        chain_2 <- current_stimuli %>% filter(Accuracy == 0) %>% select(c("participant", "AOI"))
        
        # extract stimulus data
        #chain_1 <- gp1[gp1$MediaName == current_stimuli_metadata$file_name, c("participant", "AOI")]
        #chain_2 <- gp2[gp2$MediaName == current_stimuli_metadata$file_name, c("participant", "AOI")]
        
        
        # get num of unique participants
        unique_participants <- as.vector(chain_1[!duplicated(chain_1$participant), "participant"])
        print(length(unique_participants))
        ...
        shuffled_distances <- randomPermutations(combined_data, settings, length(unique_participants), current_stimuli_metadata)
