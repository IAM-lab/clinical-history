...
initialize <- function()
{
    ...
    package.args <- c("ggplot2", "crayon", "dplyr", "tidyr", "grDevices", "lattice", "msm")
...
main <- function()
{
    ...
    for(study_comparisons in 1:5)
    {
        ...
        for(i in 1:length(getStimuliList()))
        {
            ...
            group_data <-  getSelectedExperimentalGroups(g1, g2, study_comparisons)
            g1 <- group_data[[1]]
            g2 <- group_data[[2]]
